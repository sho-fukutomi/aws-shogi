<?php
class Fdcsys2Controller extends AppController {
    ...
    public function popopo(){
        ...
        $ListLists = $this->getlist_and_adjust();

        $membersLists = $this->getMemberlistComplete();

        $cardLists = $this->getCardListFromTrello();
        $membersCards = $this->getMembersCards($cardLists);

        $cardlistById = $this->makeCardListById($cardLists,$ListLists,$membersLists);


        foreach ($membersLists as $key => $value) {
            if(isset($membersCards[$key]['cards'])){
                $membersLists[$key]['cards'] = $membersCards[$key]['cards'];
            }
        }

        foreach ($cardlistById as $key => $value) {
            if(!empty($value['member_in_charge'])){
                foreach ($value['member_in_charge'] as $key2 => $value2) {
                    //debug($value2['id']);
                    $membersLists[$value2['id']]['incharge'][] = $value;
                }
            }
        }
        ...
        $this->set('cardlistById',$cardlistById);
        $this->layout = '';
    }

    public function listedit(){
        $cardList = $this->getListFromTrello();
        $cardListFromDB = $this->fdc_list_list->find('all');
        $listOnlyDb = array();
        $newList = array();

        //トレロから取得したリストをidをキーにして整形
        foreach ($cardList as $key => $value) {
            $listFromTrello[$value['id']] = $value;
        }

        //DBに入ってるリストをチェック、トレロ上にあるか？
        foreach ($cardListFromDB as $key => $value) {
            if(isset($listFromTrello[$value['fdc_list_list']['list_id']])){
                //あったらidをキーにして整形
                $listFromDb[$value['fdc_list_list']['list_id']] = $value['fdc_list_list'];
            }else{
                //なかったらDBだけにしかなかったよリストに追加
                $value['fdc_list_list']['using_status'] = 0;
                $listOnlyDb[$value['fdc_list_list']['list_id']] = $value['fdc_list_list'];

            }
        }
        //トレロ上のリストとDBを比較、DBにあるやつは最新の情報に更新
        //DBにないリスト=新しいリスト なのでそれをDBになかったよリストに追加
        foreach ($listFromTrello as $key => $value) {
            if(isset($listFromDb[$key])){
                $listFromDb[$key]['name'] = $value['name'];
                $listFromDb[$key]['closed'] = $value['closed'];
                // $listFromDb[$key]['id_board'] = $value['id_board'];
                $listFromDb[$key]['pos'] = $value['pos'];
                $listFromDb[$key]['subscribed'] = $value['subscribed'];
                $listFromDb[$key]['softLimit'] = $value['softLimit'];
                $listFromDb[$key]['using_status'] = 1;

            }else{
                //DBになかったよリストに追加
                $value['using_status'] = 1;
                $newList[$value['id']] = $value;
            }
        }

        foreach ($listFromDb as $key => $value) {
            $sort[$key] = $value['pos'];
        }
        array_multisort($sort, SORT_ASC, $listFromDb);
        ...
        $this->set('listFromDb',$listFromDb);
        $this->set('newList',$newList);
        $this->set('listOnlyDb',$listOnlyDb);
        $this->set('teamCategory',$teamCategory);
        $this->set('workStatuslist',$workStatuslist);
        $this->layout = '';
    ...
    public function listupdate(){
        ...
        $this->layout = '';
        $target_list = $this->fdc_list_list->find('first' , array(
            'conditions' => array(
                'list_id' => $_POST['id']
            )
        ));
        if(!empty($target_list)){
            $target_list['fdc_list_list']['position_in_charge'] = $_POST['position_in_charge'];
            $target_list['fdc_list_list']['work_status'] = $_POST['work_status'];
            // $this->fdc_list_list->save($target_list);
            // var_dump($target_list);

        }else{
            $list_list =$this->getListFromTrello();
            foreach ($list_list as $key => $value) {
                if($value['id'] == $_POST['id']){
                    $target_list['fdc_list_list']['list_id'] = $value['id'];
                    $target_list['fdc_list_list']['name'] = $value['name'];
                    $target_list['fdc_list_list']['idBoard'] = $value['idBoard'];
                    $target_list['fdc_list_list']['pos'] = $value['pos'];
                    $target_list['fdc_list_list']['subscribed'] = $value['subscribed'];
                    $target_list['fdc_list_list']['softLimit'] = $value['softLimit'];
                    $target_list['fdc_list_list']['position_in_charge'] = $_POST['position_in_charge'];
                    $target_list['fdc_list_list']['work_status'] = $_POST['work_status'];
                }
            }
    ...
    public function cardhistory($cardID){
        ...
        foreach ($result as $key => $action) {
            if($action['type'] == "updateCard"){
                ...
                'listBefore' => $ListLists[$action['data']['listBefore']['id']],
                'listAfter' => $ListLists[$action['data']['listAfter']['id']],
        ...
        foreach ($cardHistory as $key => $value) {
            if($key+1 < $countHistory){
                ...
                if(isset($positionTime[$value['listAfter']['position_in_charge']][$value['listAfter']['work_status']])){
                    $positionTime[$value['listAfter']['position_in_charge']][$value['listAfter']['work_status']] = $positionTime[$value['listAfter']['position_in_charge']][$value['listAfter']['work_status']] + $actualTimediff;
                }else{
                    $positionTime[$value['listAfter']['position_in_charge']][$value['listAfter']['work_status']] = $actualTimediff;
                }

                if(isset($listTime[$value['listAfter']['name']])){

                    $listTime[$value['listId']]['actTimediff'] = $listTime[$value['listId']]['actTimediff'] + $actualTimediff;
                }else{
                    $listTime[$value['listId']]['actTimediff'] = $actualTimediff;
                    $listTime[$value['listId']]['listinfo'] = $ListLists[$value['listId']];
                }
        ...
        foreach ($positionTime as $key => $value) {
            foreach ($value as $key2 => $value2) {
                $workstatus = $key2==1 ? "working" : "waiting";
                $categoryTime[$category[$key]][$workstatus] = $this->msecTotime($value2);
                $categoryTime[$category[$key]]["key"] = $key;
            }
        }
        foreach ($listTime as $key => $value) {
            $listTime[$key]['actTimediff'] = $this->msecTotime($value['actTimediff']);
        }

//debug($listTime);
        $this->set('cardHistory',$cardHistory);
        $this->set('cardName',$result[0]['data']['card']['name']);
        $this->set('categoryTime',$categoryTime);
        $this->set('listTime',$listTime);
    ...
    private function makeCardListById($cardLists,$ListLists,$membersLists){
        //debug($ListLists);
        $firstflg = 1;
        ...
        foreach ($cardLists as $key => $value) {
            ...
            foreach ($value['idMembers'] as $key => $memberId) {
                // debug($membersLists[$memberId]['team_detail']['category']);
                // debug($ListLists[$value['idList']]['position_in_charge']);
                if(isset($ListLists[$value['idList']]['position_in_charge'])){
                    if($membersLists[$memberId]['team_detail']['category'] == $ListLists[$value['idList']]['position_in_charge'] ){
                        $member_in_charge[] = array(
                            'id' => $membersLists[$memberId]['id'],
                            'name' => $membersLists[$memberId]['fullName'],
                            'team_name' => $membersLists[$memberId]['team_detail']['name'],
                            'team_category' => $membersLists[$memberId]['team_detail']['category']
                        );

                    }
                }else{
                    debug($ListLists[$value['idList']]);
                }

                $member[] = array(
                    'name' => $membersLists[$memberId]['fullName'],
                    'team' => $membersLists[$memberId]['team'],
                    'team_name' => $membersLists[$memberId]['team_detail']['name'],
                    'team_category' => $membersLists[$memberId]['team_detail']['category']
                );
            }
            ...
            $result[$value['id']] = array(
                'name' => $value['name'],
                'list_id' => $value['idList'],
                'list' => $ListLists[$value['idList']],
                'member_ids' => $value['idMembers'],
                'members' => $member,
...
//and more
    ...
    private function getMembersCards($cardLists){
        // debug($cardLists);


        foreach ($cardLists as $key => $card) {
            foreach ($card['idMembers'] as $key => $memberID) {
                $membersCards[$memberID]['cards'][] =  $card['id'];
            }
        }
        // debug($membersCards);
        return $membersCards;
    ...
    private function getMemberlistComplete(){
        $teams = $this->fdc_team->find("all");
        foreach ($teams as $key => $value) {
            $result_team[$value['fdc_team']['id']] = array(
                "name" => $value['fdc_team']['name'],
                "category" => $value['fdc_team']['category']
            );
        }

        $membersfromDB = $this->fdc_members->find('all');
        foreach ($membersfromDB as $key => $value) {
             $result_member[$value['fdc_members']['id']] = $value['fdc_members'];
             $result_member[$value['fdc_members']['id']]['team_detail'] = $result_team[$value['fdc_members']['team']];

        }
    //    debug($result_member);

        $membersLists = $this->getMemberlistFromTrello();
        foreach ($membersLists as $key => $value) {
            $resultFromTrello[$value['id']] = $result_member[$value['id']];

        }


        foreach ($resultFromTrello as $key => $value) {
            $sort1[$key] = $value['team_detail']['category'];
            $sort2[$key] = $value['team_detail']['name'];
        }
        array_multisort($sort1,$sort2, SORT_ASC, $resultFromTrello);

        return $resultFromTrello;

    }

    private function getlist_and_adjust(){
        $cardList = $this->getListFromTrello();
        $cardListFromDB = $this->fdc_list_list->find('all');
        foreach ($cardListFromDB as $key => $value) {
            $listById[$value['fdc_list_list']['list_id']] =  $value['fdc_list_list'];
        }
        ...
        array_multisort($sort, SORT_ASC, $cardList);
        // debug($cardList);
        foreach ($cardList as $key => $value) {
            $result[$value['id']]['name'] = $value['name'];
            if(isset($listById[$value['id']]['position_in_charge'])){
                $result[$value['id']]['position_in_charge'] = $listById[$value['id']]['position_in_charge'];
                $result[$value['id']]['work_status'] = $listById[$value['id']]['work_status'];
            }else{
                debug("error! :".$value['name']);
            }
        }
    ...
    private function getListFromTrello(){
